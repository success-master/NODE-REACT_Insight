...
class CompanyService {
    getAllCompanies = () => {
        ...
        return axios.get(`${api}/admin/companys/getCompanyList`, { headers: { "Authorization": `Bearer ${token}` } })
            .then(res => res.data)
    ...
    getCompanyUsers = (companyId) => {
        const api = getInsightBackendAPI();
        const token = window.localStorage.getItem('access_token');
        return axios.get(`${api}/admin/companys/getCompanyById/` + companyId, { headers: { "Authorization": `Bearer ${token}` } })
            .then(res => res)
            .catch(error => handleErrorResponseObject(error));
    }

    createCompany = (currentTeamNameVal, currentResultMembers, currentMemberRoles) => {
        const token = window.localStorage.getItem('access_token');
        ...
        return insightBackendAPI.post('/admin/companys/companyCreate', {
    ...
    updateCompany = (companyId, currentResultMembers, selectedRoleOptions) => {
        ...
        return insightBackendAPI.post('/admin/companys/companyUpdate', {
            "companyId": companyId,
            "companyMember": currentResultMembers,
